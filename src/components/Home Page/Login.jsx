...
const Login = () => {
  ...
  // Add generateRandomString function at the top of the file
  ...
  // Update handleLinkedInLogin function in the Login component
  const handleLinkedInLogin = () => {
    const state = generateRandomString();
    const redirectUri = encodeURIComponent(
      "http://localhost:5173/auth/linkedin/callback"
    ); // Ensure this matches LinkedIn App settings
    localStorage.setItem("linkedin_oauth_state", state);
    localStorage.setItem("linkedin_role", role);

    const linkedInUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=864irfldmtcir8
&redirect_uri=${redirectUri}&state=${state}&scope=r_liteprofile%20r_emailaddress`;

    try {
      window.location.href = linkedInUrl;
    } catch (error) {
      console.error("LinkedIn Login Error:", error);
      toast.error("LinkedIn is currently unavailable. Please try again later.");
    }
  };
